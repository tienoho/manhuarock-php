...
class workerUploadBunnyCdn {
	...
	public function handle() {
		...
		$started = time();
		$this->_client = new Client($this->_config['bunny']['access_key'], $this->_config['bunny']['storage_name'], $this->_config['bunny']['storageZoneRegion']);
		...
		foreach ($this->_job as $job) {
			...
			try {
				...
				if (!is_array($this->_dataHandler) || !$this->_dataHandler) {
				...
				foreach ($imageList as $img) {
					...
					$upload = $this->_client->upload('../../publish/uploads/' . $directoryCdn, $directoryCdn);
                    $respon = json_decode($upload);
					if ($respon->Message == 'File uploaded.') {
						$finalUrl[] = $this->_config['bunny']['site_url_cdn'] . '/' . $directoryCdn;
					}
				}
