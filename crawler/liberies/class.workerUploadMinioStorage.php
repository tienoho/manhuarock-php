...
class workerUploadMinioCdn {
	...
	public function handle() {
		...
		$this->_s3 = new S3Client([
            'region' => $this->_config['Minio']['region'],
            'version' => 'latest',
            'endpoint' => $this->_config['Minio']['endpoint'],
            'use_path_style_endpoint' => true,
            'use_aws_shared_config_files' => false,
            'credentials' => [
                'key' => $this->_config['Minio']['access_key'],
                'secret' => $this->_config['Minio']['secret_key']
            ]
        ]);
			...
			try {
				...
				foreach ($imageList as $img) {
					...
					$upload = $this->_s3->putObject([
                        'Bucket' => $this->_config['Minio']['bucket_name'],
                        'Key' => $directoryCdn,
                        'ACL' => 'public-read',
					...
					if ($url) {
						//$finalUrl[] = str_replace(sprintf("%s/%s", $this->_config['Minio']['endpoint'], $this->_config['Minio']['bucket_name']), $this->_config['Minio']['site_url_cdn'], $url);
						$finalUrl[] = $url;
					}
				}
