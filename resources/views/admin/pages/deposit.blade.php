                                ...
                                <form class="mb-2" type="POST" action="{{ url(null, ['page' => 1]) }}">
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" name="search"
                                            placeholder="Điền mã giao dịch hoặc ID tài khoản để tìm kiếm"  value="{{ input()->value('search') }}">
                                        ...
                                        <div class="col-sm-12 table-responsive">
                                            <table id="table"
                                                ...
                                                @foreach($deposits as $deposit)
                                                    <input hidden name="deposit_id" id="deposit_id" value="{{$deposit->deposit_id}}">
                                                    <tr class="user-item" data-depositid="{{ $deposit-> deposit_id }}" data-id="{{ $deposit-> user_id }}" data-amount="{{ number_format($deposit->amount, 0, '', '') }}">
                                                        <td width="30px" class="text-center align-middle"><input
                                                                    type="checkbox"
                                                                    class="choose_box">
                                                        </td>
                                                        <td><span class="text-info">#{{ $deposit->code }}</span></td>
                                                        <td class="text-center"><span>{{ $deposit->name }}</span></td>
                                                        <td class="text-center"><span>{{ $deposit->email }}</span></td>
                                                        <td class="text-center"><span>{{ $deposit->type }}</span></td>
                                                        <td class="text-center" ><span>{{ number_format($deposit->amount, 0, '', '.') }}</span></td>
                                                        ...
                                                        <td class="text-center" width="200px">
                                                            @if($deposit->status == 'pending')
                                                              <button type="button" class="btn btn-info btn-sm add-coin">
                                                                ...
                                                                <button type="button" class="btn btn-danger btn-sm reject" onclick="Reject({{$deposit->id}})">
                                                                    Từ chối
                                                                </button>
                                                            @else
                            ...
                            <div class="card-footer clearfix">
                                <ul class="pagination m-0 float-right">
                                    <li class="page-item {{ $page <= 1 ? 'disabled' : '' }}">
                                        <a class="page-link " href="{{ url(null, ['page' => $page - 1], ['search' => input()->value('search')]) }}">«</a>
                                    </li>
                                    @if($page > 1)
                                        <li class="page-item">
                                            <a class="page-link"
                                               href="{{ url(null, ['page' => $page - 1], ['search' => input()->value('search')]) }}">{{ $page - 1 }}</a>
                                        </li>
                                    @endif
                                    ...
                                    @if($page + 2 <= $total_page)
                                        <li class="page-item">
                                            <a class="page-link"
                                               href="{{ url(null, ['page' => $page + 2, ], ['search' => input()->value('search')]) }}">{{ $page + 2 }}</a>
                                        </li>
                                    @endif
