...
$content =  file_get_contents($file_crawler);
$content =  reverse_string(base64_encode($content));
?>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit campaign: <?php echo e($name); ?></h5>
            ...
            <div class="modal-body">
                <div class="row mb-3 align-items-end">
                    <div class="col">
                        ...
                        <input type="text" id="name" class="form-control" name="name" value="<?php echo e($name); ?>"/>
                    </div>
                </div>
                <div>
                    <label class="form-label">Campaign Code</label>
                    <textarea class="form-control" id="content" rows="10" name="content"><?php echo e($content); ?></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn me-auto" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary" id="edit-campaign">Submit Change</button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $("#edit-campaign").click(function () {
        ...
        let data = {
            current_path: "/backend/Crawler/<?php echo e($name); ?>.php",
            name: $modal_ajax.find("#name").val(),
            content: $modal_ajax.find("#content").val(),
        };

        $.post(base_url + '/edit-campaign', data).done(function (res) {
            if(res.status){
...
</script><?php /**PATH /www/wwwroot/mangarock.top/resources/views/autosite/template/modal-edit-campaign.blade.php ENDPATH**/ ?>